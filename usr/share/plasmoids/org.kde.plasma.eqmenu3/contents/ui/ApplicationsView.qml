...
Item {
    ...
    function activateCurrentIndex(start) {
        if (gridView.totalColumns() == 1) {
            ...
            if (start) {
                ...
                return applicationsView.activateCurrentIndex();
            }
        }
    }
    ...
    function deactivateCurrentIndex(exit) {
        if (gridView.totalColumns() == 1) {
    ...
    KickoffGridView {
        ...
        function moveLeft() {
            ...
            gridView.currentIndex = oldModelIndex;
            gridView.positionViewAtIndex(oldModelIndex, GridView.Center);
        }

        function moveRight() {
            state = "";
            activatedItem.activate()
            applicationsView.gridView.positionViewAtBeginning()
        }
